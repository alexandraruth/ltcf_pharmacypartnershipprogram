#########################################################
### This script: 
#      1. Cleans the Hayes Directories pharmacy list data and merges it with county-level FIPS codes for each pharmacy by matching county strings
#      2. Subsets a df containing only CVS and Walgreens pharmacy locations

# Inputs:
#  - "hayes_2021.csv" (Raw Hayes Directories pharmacy list)

# Outputs:
#  - "hayes_cty.Rda" (df with county fips codes linked to pharmacies)
#  - "cvs_wal.Rda" (df with only CVS and Walgreens locations)
#  - "cvswal_count.Rda" (df with state-level counts of CVS and Walgreens locations)


# Note: This was a very verbose and clunky way to do this, out of necessity / time pressure! In the future, to correct the string misspellings and discrepancies, I would use some of the following approaches:
#       1. Use fuzzy match with the stringdist package to match the ones that are minor misspellings
#       2. Create a separate csv listing out the misspellings and necessary revisions, then use this df to correct errors and keep a log of changes made
...
## PART I: Clean Hayes Directories data ----
...
dnm_hayes <- hayes %>% # selects the rows that did not match for inspection
...
## PART II: Get CVS and Walgreens; keep list of ONLY all CVS & Walgreens locations ----
...
# save state-level pharmacy counts df 
